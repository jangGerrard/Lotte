...
public class SttUIActivity extends Activity {
    ...
    private final Handler mHandler = new SttHandler(this);

    // 메모리 누수 없애기 위한 Inner Class
    private static class SttHandler extends Handler{
        int i = 0 ;
        private final int READY = 0, END=1, FINISH=2;			//핸들러 메시지. 음성인식 준비, 끝, 앱 종료
        private final WeakReference<SttUIActivity> sActivity;   // 메모리 누수 제거용 WeakReference
        public SttHandler(SttUIActivity activity){
            sActivity = new WeakReference<SttUIActivity>(activity);
        }
        @Override
        public void handleMessage(Message msg) {
            SttUIActivity activity = sActivity.get();
            if (activity != null) {
                switch (msg.what) {
                    case READY:
                        activity.mProgress.setVisibility(View.INVISIBLE);				//준비되었으면 프로그레스바 감춤
                        activity.findViewById(R.id.stt_ui).setVisibility(View.VISIBLE);	//마이크 이미지 보임.
                        break;
                    case END:
                        activity.mProgress.setVisibility(View.VISIBLE);						//말이 끝났으면 프로그레스바 출력(음성인식 중)
                        activity.findViewById(R.id.stt_ui).setVisibility(View.INVISIBLE);	//마이크 이미지 감춤
